import { getStrokeCount } from "@/lib/analysis/hanja-strokes"
...
const INTERPRETATION_RANGES = [
  {
    max: 10,
    text: "기초 운이 약해 환경의 뒷받침이 필요합니다",
  },
  {
    max: 20,
    text: "안정과 성실이 돋보이며 꾸준함이 힘이 됩니다",
  },
  {
    max: 30,
    text: "도전과 성취의 기운이 있어 목표 달성에 유리합니다",
  },
  {
    max: 40,
    text: "책임감과 리더십이 강해 주변 신뢰를 얻습니다",
  },
  {
    max: 50,
    text: "활동성이 높고 대인관계에서 에너지가 빛납니다",
  },
  {
    max: 60,
    text: "균형감이 좋고 장기 계획에 강점을 보입니다",
  },
  {
    max: 70,
    text: "현실 감각과 실무 능력이 뛰어나 기반을 다집니다",
  },
  {
    max: 81,
    text: "완성의 기운이 강해 성취와 명예를 함께 얻습니다",
  },
]
...
function deriveOverallInterpretation(grids: NameNumerologyResult["grids"]) {
  const average = Math.round(
    (grids.won + grids.hyung + grids.yi + grids.jeong) / 4
  )
...
export function calculateNameNumerology(input: {
  ...
  if (!split) {
    return {
      status: "invalid-name",
      message: "이름은 2~4글자 한글로 입력해야 합니다.",
    }
  }

  if (!input.hanjaName) {
    return {
      status: "missing-hanja",
      message: "선택된 한자가 없습니다.",
      missingSyllables: splitSyllables(name),
    }
  }
  ...
  if (nameSyllables.length !== hanjaSyllables.length) {
    return {
      status: "missing-hanja",
      message: "한자 선택이 이름 길이와 일치하지 않습니다.",
      missingSyllables: nameSyllables,
    }
  }
  ...
  if (missingHanja.length > 0) {
    return {
      status: "unknown-stroke",
      message: "등록되지 않은 한자 획수가 포함되어 있습니다.",
      missingHanja,
    }
  }

  const surnameStrokes = strokes
    .slice(0, split.surnameLength)
    .reduce((sum, value) => sum + value, 0)
  const givenStrokes = strokes
    .slice(split.surnameLength)
    .reduce((sum, value) => sum + value, 0)
