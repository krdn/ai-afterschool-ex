/**
 * 별자리(Zodiac) 매핑 및 기본 데이터
 * 생년월일로 12별자리를 결정하고 기본 특성 정보를 제공합니다.
 */

export type ZodiacSign = {
  key: string       // 영문 키 (예: "aries")
  name: string      // 한글명 (예: "양자리")
  symbol: string    // 기호
  element: string   // 원소 (fire/earth/air/water)
  elementName: string // 원소 한글명
  rulingPlanet: string  // 지배 행성
  dateRange: string  // 날짜 범위 표시용
  traits: string[]   // 주요 성격 특성
  strengths: string[]
  weaknesses: string[]
  learningStyle: string  // 학습 스타일 요약
}
...
// 월/일 기준 별자리 경계 (각 별자리의 시작 월/일)
const ZODIAC_BOUNDARIES: Array<{ month: number; day: number; index: number }> = [
  { month: 1, day: 20, index: 1 },   // 물병
  { month: 2, day: 19, index: 2 },   // 물고기
  { month: 3, day: 21, index: 3 },   // 양
  { month: 4, day: 20, index: 4 },   // 황소
  { month: 5, day: 21, index: 5 },   // 쌍둥이
  { month: 6, day: 22, index: 6 },   // 게
  { month: 7, day: 23, index: 7 },   // 사자
  { month: 8, day: 23, index: 8 },   // 처녀
  { month: 9, day: 23, index: 9 },   // 천칭
  { month: 10, day: 23, index: 10 }, // 전갈
  { month: 11, day: 22, index: 11 }, // 궁수
  { month: 12, day: 22, index: 0 },  // 염소
]
...
export function getZodiacSign(birthDate: Date): ZodiacSign {
  ...
  // 경계를 역순으로 순회하여 현재 날짜가 속하는 별자리 결정
