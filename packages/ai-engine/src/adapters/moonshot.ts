...
export class MoonshotAdapter extends BaseAdapter {
  ...
  createModel(modelId: string, config?: ProviderConfig): LanguageModel {
    ...
    return moonshot.chatModel(modelId) as unknown as LanguageModel
  }
  ...
  async validate(config: ProviderConfig): Promise<ValidationResult> {
    try {
      ...
      if (
        ...
        return { isValid: true, error: "API 키는 유효하지만 잔액이 부족합니다. 충전 후 이용하세요." }
      }
  ...
  async listModels(config: ProviderConfig): Promise<ModelInfo[]> {
    try {
      ...
      const data = (await response.json()) as {
        data?: Array<{ id: string; context_length?: number; supports_image_in?: boolean }>
      }
  ...
  private getDefaultModels(): ModelInfo[] {
    return [
      { id: "kimi-k2.5", modelId: "kimi-k2.5", displayName: "Kimi K2.5", contextWindow: 262144, supportsVision: true, supportsTools: true },
      { id: "kimi-k2-thinking", modelId: "kimi-k2-thinking", displayName: "Kimi K2 Thinking", contextWindow: 262144, supportsVision: false, supportsTools: true },
      { id: "kimi-latest", modelId: "kimi-latest", displayName: "Kimi Latest", contextWindow: 131072, supportsVision: true, supportsTools: true },
      { id: "moonshot-v1-8k", modelId: "moonshot-v1-8k", displayName: "Moonshot 8K", contextWindow: 8192, supportsVision: false, supportsTools: true },
      { id: "moonshot-v1-32k", modelId: "moonshot-v1-32k", displayName: "Moonshot 32K", contextWindow: 32768, supportsVision: false, supportsTools: true },
      { id: "moonshot-v1-128k", modelId: "moonshot-v1-128k", displayName: "Moonshot 128K", contextWindow: 131072, supportsVision: false, supportsTools: true },
    ]
  }
  ...
  protected getDefaultBaseUrl(): string { return "https://api.moonshot.ai/v1" }
}
