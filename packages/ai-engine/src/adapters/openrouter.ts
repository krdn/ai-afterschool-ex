...
export class OpenRouterAdapter extends BaseAdapter {
  ...
  createModel(modelId: string, config?: ProviderConfig): LanguageModel {
    ...
    return openrouter.chatModel(modelId) as unknown as LanguageModel
  }
  ...
  async validate(config: ProviderConfig): Promise<ValidationResult> {
    try {
      ...
      if (response.status === 401) return { isValid: false, error: "API 키가 유효하지 않습니다." }
      if (response.status === 402) return { isValid: true, error: "API 키는 유효하지만 크레딧이 부족합니다. 충전 후 이용하세요." }
      return { isValid: false, error: `검증 실패 (HTTP ${response.status})` }
    } catch (error) {
  ...
  protected getDefaultBaseUrl(): string { return "https://openrouter.ai/api/v1" }
}
